...
**注意事项**：
- fallbacks 中的模型上下文窗口应该 ≥ 主模型
- Hunyuan-T1 只有 32K，不适合作为长对话的 fallback
...
| **混元系列** |||
| Hunyuan-T1 | **32K** | 8K |
| Hunyuan-TurboS | 128K | 8K |
| Hunyuan-2.0-Instruct | 128K | 16K |
| Hunyuan-2.0-Thinking | 128K | 64K |
| **DeepSeek** |||
...
- **Hunyuan-T1 只有 32K**，不适合作为长对话的 fallback
- **上下文窗口配置错误会导致频繁触发 "Context limit exceeded"**
- **fallbacks 中的模型上下文窗口应该 ≥ 主模型**
...
6. 启动 memUbot 守护进程
7. 复制 skills/ 目录（技能体系）
8. 复制 USER.md + AGENTS.md（核心准则 + 会话配置）
9. 复制 MEMORY.md + SOUL.md（长期记忆 + 身份）
```
